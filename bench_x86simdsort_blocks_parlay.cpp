...
static bool parse_dtype(const std::string& s, DType& out) {
  ...
  if (s == "int63")  { out = DType::INT64;  return true; } // alias
...
struct Args {
  ...
  // Parlay options
  int threads = 0;          // 0 => use Parlay default
  size_t grain = 0;         // 0 => Parlay decides
};
...
static void usage(const char* prog) {
  std::cerr
    ...
    << " --descending 0 --check 0 --seed 12345 --threads 0 --grain 0\n";
}
...
static bool parse_args(int argc, char** argv, Args& a) {
  for (int i = 1; i < argc; i++) {
    ...
    if (k == "--type") {
      ...
      DType dt;
      if (!parse_dtype(v, dt)) { std::cerr << "Bad --type: " << v << "\n"; return false; }
      a.dtype = dt;
      ...
      const char* v = need_val("--grain"); if (!v) return false;
      a.grain = std::stoull(v);
...
template <class T>
static int bench_blocks_parlay(const Args& args, const char* dt_name) {
//   if (args.threads > 0) {
//     parlay::set_num_workers(args.threads);
//   }
  ...
  // Keep CSV schema compatible with your python:
  ...
  // blocks: min, *2, ... plus max if needed
  ...
  for (size_t block : blocks) {
    ...
    // same input for this block size round
    std::memcpy(work.data(), master.data(), effective_n * sizeof(T));

    // Parallel sort each block
    ...
    // Optional correctness spot-check after parallel region
    if (args.check) {
      for (size_t bi = 0; bi < num_blocks; bi += 1024) {
        T* ptr = work.data() + bi * block;
...
int main(int argc, char** argv) {
  ...
  switch (args.dtype) {
    case DType::INT32:  return bench_blocks_parlay<int32_t>(args, dt_name);
    case DType::INT64:  return bench_blocks_parlay<int64_t>(args, dt_name);
    case DType::FLOAT:  return bench_blocks_parlay<float>(args, dt_name);
    case DType::DOUBLE: return bench_blocks_parlay<double>(args, dt_name);
  }
